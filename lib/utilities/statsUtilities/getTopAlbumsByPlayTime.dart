...
Map<String, double> getTopAlbumsByPlayTime(AppState appstate,
  ...
  for (var i = 0; i < appstate.artists.length; i++) {
    for (var j = 0; j < appstate.artists[i].albums.length; j++) {
      ...
      if (!showCounts)
        ...
        albums.add({
          "title": appstate.artists[i].albums[j].title + " (" + totalClicks.toInt().toString() + ")",
          "artist": appstate.artists[i].name,
          "totalClicks": totalClicks
        });
    }
  ...
  albums.sort((a, b) => b["totalClicks"].compareTo(a["totalClicks"].toDouble()));
  ...
  double otherAlbum = 0; // when not shown in my album map primarily, it is shown in otherAlbum group
  double topAlbum = 0;
  ...
  for (int i = 0; i < albums.length; i++) {
    ...
    if (totalStatistics == 0 && appstate.settings.statisticNumberAlbum > i) {
      albumObj.putIfAbsent(albums[i]["title"] + " - " + albums[i]["artist"], () => albums[i]["totalClicks"]);
  ...
  if (totalStatistics == 0 && albums.length > appstate.settings.statisticNumberAlbum) {
