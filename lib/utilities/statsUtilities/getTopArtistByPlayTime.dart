...
Map<String, double> getTopArtistByPlayTime(AppState appstate,
  ...
  for (var i = 0; i < appstate.artists.length; i++) {
    ...
    double totalClicks = appstate.artists[i].albums
        .map((e) => e.songs)
        .map((e) => e.map((e) => e.playCount).reduce((value, element) => value += element).toDouble())
        .reduce((value, element) => value += element);
    if (!showCounts)
      myartists.add({"artist": appstate.artists[i].name, "totalClicks": totalClicks});
    else
      myartists.add({
        "artist": appstate.artists[i].name + " (" + totalClicks.toInt().toString() + ")",
        "totalClicks": totalClicks
      });
  }

  /// Compare total played seconds for each artist, then sort by total seconds
  myartists.sort((a, b) => b["totalClicks"].compareTo(a["totalClicks"].toDouble()));
  ...
  double otherArtist = 0; // when not shown in my artist map primarily, it is shown in otherArtist group
  double topArtist = 0;
  ...
  for (int i = 0; i < myartists.length; i++) {
    ...
    if (totalStatistics == 0 && appstate.settings.statisticNumberArtist > i) {
      myartistsObj.putIfAbsent(myartists[i]["artist"], () => myartists[i]["totalClicks"]);
  ...
  if (totalStatistics == 0 && myartists.length > appstate.settings.statisticNumberArtist) {
